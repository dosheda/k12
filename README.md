...
![LLM](https://img.shields.io/badge/LLM-DeepSeek-111827)
![Local First](https://img.shields.io/badge/data-local--first-0f766e)

## 一句话简介

这是一个可以在本机或同一局域网运行的 **K12 古诗词 RAG 学习助手**。学生像聊天一样提问，系统先从本地古诗词知识库找到相关诗歌，再结合最近对话历史生成分板块讲解；侧边栏会记录学习概览，并支持生成给家长/老师看的学情报告。

## 功能亮点

| 能力 | 说明 |
|---|---|
| 本地优先 | 古诗词知识库、Chroma 向量库和 SQLite 学习记录都落在本机，适合个人演示和同一局域网试用。 |
| RAG 闭环完整 | 包含古诗文本整理、向量建库、标签生成、混合检索、对话讲解、学习记录和报告生成。 |
| 混合检索 | 语义向量检索结合关键词与主题标签，缓解“问法不同就搜不到”的问题。 |
| 学生友好讲解 | Prompt 约束为耐心、清楚、分板块的讲解方式，避免只给成人化赏析。 |
| 多轮对话 | 支持“再讲一首”“它的作者是谁”这类接续追问，并能恢复最近 5 轮聊天。 |
| 学情分析 | 区分真正讲解、复习、仅提及和候选诗，减少误统计，并可生成学情报告。 |
| 局域网访问保护 | Streamlit 页面要求访问口令，浏览器可选择记住 7 天，避免局域网裸奔。 |

## 效果预览

### 聊天讲解

学生输入自然语言问题后，小 K 老师会先提示检索到的诗歌，再给出内容、情感和写法讲解。

![聊天讲解效果](docs/screenshots/chat-overview.png)

### 学习概览与学情入口

侧边栏持续展示学习统计、覆盖主题和高频主题，学情报告可在弹窗中查看，也可以重新生成。
