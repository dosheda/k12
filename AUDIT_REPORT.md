...
审计范围：审计时项目根目录的源码、文档、依赖声明、运行时数据文件和可见目录状态。审计过程未修改业务代码，仅生成本报告。
...
整改状态更新（2026-06-13，准备发布 `v0.2.0`）：
- 已修 P1：Streamlit 局域网访问增加 `K12_HELPER_ACCESS_CODE` 口令；聊天 API 调用增加输入长度限制和冷却限制。
- 已修 P1：硬编码项目路径集中到 `config.py`，默认基于项目根目录，并支持环境变量覆盖。
- 已修 P1：删除/覆盖类脚本改为先备份或原子替换；建库脚本不再静默删除旧 `chroma_db/`。
- 已修 P1：学习记录 JSON 损坏、Chroma 空结果/缺字段/记录数不足等崩溃点已增加保护。
- 已修 P2：API 响应结构校验和用户侧安全错误文案集中到 `api_utils.py`。
- 已修 P2：使用手册不再建议把 API key 写入 `.bat`；文档改为系统环境变量方式，并说明学习数据会发送给 DeepSeek API。
- 已修 P2：OCR 图片增加文件大小和像素限制；学情报告增加记录数、字段长度和 prompt 长度限制；直接依赖已固定版本并移除疑似未使用的 `langchain*` 依赖。
- 遗留 P2：`chromadb==1.5.9` 仍命中 CVE，但截至本次整改未查到更高修复版；当前继续以“只允许本地 PersistentClient、不运行 HTTP server、不开放端口”作为硬红线。
- 遗留 P2/P3：真实学生数据接入前仍需正式隐私说明/同意流程/数据保留策略；RAG 逻辑重复、`app.py` 过长、README/测试/CI 缺失仍待后续处理。
...
- 当前代码还把数据库路径硬编码到旧项目目录下的 `learning_records.db`，与审计时项目根目录不一致，容易出现“代码读写另一个目录的数据”的问题。
...
说明：
- 多处路径写死为旧项目目录或用户个人目录，与当前项目目录不一致。
