...
当前阶段是安全与质量整改前的整理阶段：已完成全面审计并补全文档规则，但代码层面的 P1/P2 问题还没有修复。
...
3. 先修 P1/P2：补 `.gitignore`，排除运行时数据、向量库、缓存、密钥文件和真实启动脚本。
4. 继续修 P1：给会删除/覆盖数据的脚本加备份、确认或 `--force`。
5. 修 P2：升级并锁定 `chromadb` 和直接依赖版本，清理疑似未使用依赖。
6. 补基础工程：README、测试目录、运行说明、隐私说明和配置示例。
...
### P2
- 【P2，真实数据前升 P1】`learning_records.db` 位于项目目录，当前是模拟数据；后续接入真实学生数据前必须迁移或明确保护。当前还没有 `.gitignore`，初始化 git 前有误提交风险。位置：`learning_db.py:38`、`learning_records.db`。
...
- 【2026-06-13】确认当前尚未初始化 git；初始化 git 前必须先补 `.gitignore`，避免运行时数据和密钥误提交。

---

## 给下一个对话的备注
- 开始任务前先读 `AGENTS.md`、`PROGRESS.md`、`AUDIT_REPORT.md`。
- 目前没有代码层 P1 已修复；优先从局域网访问鉴权、硬编码路径、`.gitignore`、数据覆盖保护开始。
- 不要把 Chroma 改成 HTTP server；当前只允许本地 `PersistentClient`。
- 不要把 API key 写入 `.bat`、`.env` 或文档真实示例。
- 当前目录不是 git 仓库；初始化 git 前先处理 `.gitignore`。
